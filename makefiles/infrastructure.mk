...
start: ## Inicia la infraestructura de entrada (red común, traefik con socket-proxy, cloudflared, adguardhome)
	...
	@cd $(SERVICES_DIR) && docker-compose up -d network-dummy
	...
	@cd $(SERVICES_DIR)/cloudflared && docker-compose up -d
	@echo "$(YELLOW)4. Iniciando AdGuard Home...$(NC)"
	@cd $(SERVICES_DIR)/adguardhome && docker-compose up -d
...
stop: ## Detiene todos los servicios de la infraestructura de entrada
	...
	@cd $(SERVICES_DIR)/adguardhome && docker-compose down
	@cd $(SERVICES_DIR)/cloudflared && docker-compose down
	@cd $(SERVICES_DIR)/traefik && docker-compose down
...
logs: ## Muestra los logs de todos los servicios
	...
	@cd $(SERVICES_DIR)/traefik && docker-compose logs --tail=50
	@echo ""
	@echo "$(YELLOW)--- Cloudflared ---$(NC)"
	@cd $(SERVICES_DIR)/cloudflared && docker-compose logs --tail=50
	@echo ""
	@echo "$(YELLOW)--- AdGuard Home ---$(NC)"
	@cd $(SERVICES_DIR)/adguardhome && docker-compose logs --tail=50

logs-follow: ## Sigue los logs de todos los servicios en tiempo real
	@echo "$(GREEN)Siguiendo logs (Ctrl+C para salir)...$(NC)"
	@cd $(SERVICES_DIR)/traefik && docker-compose logs -f &
	@cd $(SERVICES_DIR)/cloudflared && docker-compose logs -f &
	@cd $(SERVICES_DIR)/adguardhome && docker-compose logs -f &
	@wait
